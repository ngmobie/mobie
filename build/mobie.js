...
BackdropFactory.$inject = [ "MbBackdrop" ], angular.module("mobie.components.backdrop", [ "mobie.core.component" ]).factory("MbBackdrop", [ "MbSimpleComponent", "$q", "$animate", function(MbSimpleComponent, $q, $animate) {
    function MbBackdrop(id) {
        MbSimpleComponent.call(this), this.queue = [], this._queuePromise = null, this.setId(id, nextId()), 
        this.setElement(this.createBackdropElement()), this.enterElement();
    }
    return inherits(MbBackdrop, MbSimpleComponent, {
        createBackdropElement: function() {
            return this.getElement() ? this.getElement() : angular.element(this.template);
        },
        template: '<div class="backdrop mb-backdrop"></div>',
        containerElement: angular.element(document.body),
        enterElement: function() {
            if (angular.isUndefined(this.getElement())) throw new Error("element does not exists");
    ...
    return {
        ...
        controller: [ "$scope", "$attrs", function($scope, $attrs) {
            $scope.classes = {
                ...
                iconName ? $scope.hasIconName = $scope.classes["fa-" + iconName] = !0 : $scope.hasIconName = !1;
            });
        } ]
    };
});
function $MbModalProvider() {
    function $MbModalFactory(MbComponent, $animate, $mbBackdrop, $timeout) {
        function MbModal(options) {
            ...
            }, this.on("visibleChangeStart", this.appendBodyClass), this.on("notVisible", this.removeBodyClass);
        }
        var bodyElement = angular.element(document.body);
        return inherits(MbModal, MbComponent, {
            appendBodyClass: function(remove) {
                return this.digest(function() {
                    $animate[remove ? "removeClass" : "addClass"](bodyElement, this.options.activeBodyClass);
                }), this;
            },
            removeBodyClass: function() {
                return this.appendBodyClass(!0);
            }
        }), angular.extend(function(options) {
...
function $MbPopupProvider() {
    function $MbPopupFactory(MbPopup, $mbBackdrop) {
        function _MbPopup() {
            MbPopup.call(this), this.component.on("element", function(element) {
                var node = element[0], ON_CLICK = function(e) {
...
angular.module("mobie.components.popup", [ "mobie.core.helpers", "mobie.core.component", "mobie.components.backdrop" ]).factory("MbPopup", [ "$rootScope", "MbComponent", "$q", "$animate", function($rootScope, MbComponent, $q, $animate) {
    function MbPopup(options) {
        ...
        this.emit("scope", scope), this.component.on("element", function(element) {
    ...
    return inherits(MbPopup, EventEmitter, {
        ...
        reset: function() {
            this.locals(this.defaultScopeAttrs), this.emit("reset");
        },
        locals: function(locals) {
            return this.component.locals(locals), this.emit("updated"), this;
        },
        ...
        show: function(options, id) {
            ...
            this.options = options, this.emit("show", options, this.id), this.reset(), this.locals(options), 
            this.digest(function() {
...
function MbSimpleComponentFactory($q, $animate, EventEmitter) {
    function MbSimpleComponent(componentEl, id, options) {
        ...
        angular.isDefined(id) && this.setId(id), this.on("visibleStateChangeSuccess", function() {
            this.getVisibleState() ? this.emit("visible") : this.emit("notVisible");
        }), this.on("visibleStateChangeStart", function(visibleState) {
            visibleState ? this.emit("visibleChangeStart") : this.emit("notVisibleChangeStart");
        });
    }
    return inherits(MbSimpleComponent, EventEmitter, {
        ...
        setVisibleState: function(visibleState) {
            ...
            return classReceiverElement ? (this.emit("visibleStateChangeStart", visibleState), 
            addClass = visibleState ? visibleClass : hiddenClass, removeClass = visibleState ? hiddenClass : visibleClass, 
            $animate.setClass(classReceiverElement, addClass, removeClass).then(function() {
                self.isVisible = visibleState, self.emit("visibleStateChangeSuccess");
            }, function(err) {
                self.emit("visibleStateChangeError", err);
            })) : $q.reject(new Error("element does not exists"));
        },
        setId: function(id) {
...
function $MbComponentProvider() {
    function $MbComponentFactory(MbSimpleComponent, $http, $q, $controller, $compile, $templateCache, $animate, $rootScope) {
        function MbComponent(componentEl, id, options) {
            ...
            !id && options.id && (id = options.id), id && this.setId(id), this.on("scope", function(scope) {
            ...
            this.emit("scope", this.scope), scope.$on("$destroy", function() {
                ...
                this.emit("componentLink", this.componentLink), this.enterElement();
            }), (this.options.templateUrl || this.options.template) && this.prepareComponent();
        }
        var bodyElement = angular.element(document.body);
        return inherits(MbComponent, MbSimpleComponent, {
            ...
            digest: function(callback, context) {
                ...
                return scope && scope.$root ? (scope.$$phase || scope.$root.$$phase ? fn && scope.$applyAsync(fn) : scope.$apply(fn), 
                this) : (fn(scope), this);
            },
            setElement: function() {
            ...
            enterElement: function() {
                var _this = this, element = this.getElement();
                return element ? (angular.isUndefined(this.parentElement) && (this.parentElement = bodyElement), 
                this.emit("enterElementStart"), $animate.enter(element, this.parentElement).then(function() {
                    _this.emit("enter");
                })) : $q.reject(new Error("element does not exists"));
            },
            removeElement: function() {
            ...
            leaveElement: function() {
                var _this = this, element = this.getElement();
                return element ? (this.emit("leaveElementStart"), $animate.leave(element).then(function() {
                    return _this.emit("leave"), _this;
                })) : $q.reject(new Error("element does not exists"));
            },
            postDigest: function(callback) {
                return this.scope.$$postDigest(callback.bind(this)), this;
            },
            destroy: function() {
                var _this = this;
                return this.emit("destroy"), this.setVisibleState(!1).then(function() {
...
MbSimpleComponentFactory.$inject = [ "$q", "$animate", "EventEmitter" ], angular.module("mobie.core.component", [ "mobie.core.helpers", "mobie.components.animation" ]).factory("MbSimpleComponent", MbSimpleComponentFactory).provider("MbComponent", $MbComponentProvider);
...
mobie.digest = digest, angular.module("mobie.core.helpers", [ "mobie.core.eventemitter" ]);
